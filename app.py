...
VALID_ACCOUNTS = {"user1": "123456", "user2": "123456", "user3": "123456"}
...
def ask_question():
    ...
    try:
        ...
        response = requests.post(
            QA_URL, headers=qa_headers, json=qa_data, stream=True, verify=False
        )
...
def verify_token(current_user):
    # 如果token有效，返回用户信息
    return jsonify({"username": current_user, "is_authenticated": True})
...
def save_history(current_user):
    ...
    existing_history = ConversationHistory.query.filter_by(
        username=current_user, conversation_id=conversation_id
    ).first()
    if existing_history:
        # 存在则更新记录
...
def get_history(current_user):
    histories = (
        ConversationHistory.query.filter_by(username=current_user)
...
def transcribe_audio():
    ...
    try:
        ...
        # 将转换后的WAV文件发送到语音接口
        files = {"file": ("converted.wav", wav_buffer, "audio/wav")}

        response = requests.post(VOICE_SERVER_URL, files=files, timeout=30)
...
def refresh_conversationid():
    ...
    try:
        # 发起流式请求
        response = requests.post(
            GT_CID_URL, headers=qa_headers, json=qa_data, verify=False
        )
        if response.status_code == 200:
...
def download_document(dataset_id, doc_id, doc_name):
    try:
        ...
        response = requests.get(
            DOC_URL, headers=doc_headers, params=params, verify=False, stream=True
        )
        response.raise_for_status()
...
def text_to_speech_stream():
    ...
    try:
        ...
        return app.response_class(generate_and_stream(), mimetype="text/event-stream")
